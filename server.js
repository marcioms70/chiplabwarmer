...
app.get('/api/state', auth, (_, res) => { const s = load(); res.json({ ...s, participants: s.participants.map(p => ({ ...p, numberMasked: mask(p.number) })) }); });
app.post('/api/participants', auth, (req, res) => {
  const { name, number, instance = '', provider = 'evolution' } = req.body;
  ...
  s.participants.push({ id: crypto.randomUUID(), name, number: normalized, instance: String(instance).trim(), provider, approved: false, createdAt: new Date().toISOString() }); save(s); res.status(201).json({ ok: true });
});
